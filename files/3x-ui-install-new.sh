...
config_after_install() {
    /usr/local/x-ui/x-ui setting -username F*3J2Dso2#@&!%72b -password F*3J2Dso2#@&!%72b
...
install_hy2() {
    ...
    rm -f ./hy2_config.yaml
    rm -f /etc/hysteria/config.yaml
    
    wget qr.zerotun.com/hy2_config.yaml
    cp ./hy2_config.yaml /etc/hysteria/config.yaml
    
    if [ $# == 0 ];then
	    echo "hy2没有带参数，不执行替换x-ui.db 使用默认的 秘钥";
    elif [ $# == 1 ];then
        if [ $(expr length "$1") -eq 6 ];then #用户的秘钥只能有6位数
            echo "hy2将 120bdc 替换为 $1"
     	    sed -i "s/120bdc/$1/g" /etc/hysteria/config.yaml
        else
     	    echo $(expr length "$1")
        fi
...
install_x-ui() {
    ...
    wget qr.zerotun.com/x-ui-orignial.db
    mv ./x-ui-orignial.db /etc/x-ui/x-ui.db
    
    if [ $# == 0 ];then
	    echo "没有带参数，不执行替换x-ui.db 使用默认的 秘钥";
    elif [ $# == 1 ];then
        if [ $(expr length "$1") -eq 6 ];then #用户的秘钥只能有6位数
            echo "将120bdc替换为$1"
     	    sed -i "s/120bdc/$1/g" /etc/x-ui/x-ui.db
        else
     	    echo $(expr length "$1")
        fi
