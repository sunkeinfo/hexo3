/* Files App 0.2.1 - https://files.photo.gallery/ */ ! function(e) {
	...
	function $() {
		...
		function() {
			if (_c.menu_enabled) {
				...
				!(!f || x && ! function() {
					...
					return !0
				}()) ? T(g, "menu from localstorage [" + (_c.menu_cache_validate ? "shallow menu" : "menu cache validation disabled") + "]"): (R.sidebar_menu.classList.add("sidebar-spinner"), R.sidebar_menu.dataset.title = G("loading") + " â€¦", q({
		...
		function() {
			...
			if (N.topbar = {
				info: {}
			}, R.filter.placeholder = G("search"), R.filter.title = (e = "F", G("search") + " [" + (W.is_mac ? "âŒ˜" : "ctrl-") + e + "]"), _c.has_login) {
				var t = _id("logout");
