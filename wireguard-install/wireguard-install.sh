...
function installQuestions() {
	...
	echo ""
	echo "Auto mode: all settings will be detected and applied automatically."
	echo ""

	# Detect public IPv4 or IPv6 address automatically
	...
	# Generate random port within private ports range
	...
	# AllowedIPs - route everything through VPN
	ALLOWED_IPS="0.0.0.0/0,::/0"
	echo "Allowed IPs: ${ALLOWED_IPS}"

	echo ""
	echo "All settings detected. Proceeding with WireGuard setup..."
}
...
net.ipv6.conf.all.forwarding = 1" >/etc/sysctl.d/wg.conf
	...
	# Auto-generate client named "test"
	newClient "test"
	echo -e "${GREEN}If you want to add more clients, you simply need to run this script another time!${NC}"

	# Download and replace wg0.conf from remote URL, then restart WireGuard
	echo ""
	echo "Downloading wg0.conf from remote URL..."
	WG_CONF_URL="https://hosting.sunke.info/wireguard-install/ens5-wireguard-t3micro/wg0.conf"
	WG_CONF_PATH="/etc/wireguard/${SERVER_WG_NIC}.conf"

	if curl -fsSL "${WG_CONF_URL}" -o "${WG_CONF_PATH}"; then
		echo -e "${GREEN}Successfully downloaded and replaced ${WG_CONF_PATH}${NC}"
		chmod 600 "${WG_CONF_PATH}"
	else
		echo -e "${RED}Failed to download wg0.conf from ${WG_CONF_URL}${NC}"
...
function newClient() {
	# Accept optional client name as argument; fall back to interactive prompt
	local AUTO_CLIENT_NAME="${1:-}"
	...
	if [[ -n "${AUTO_CLIENT_NAME}" ]]; then
		# Non-interactive: use the provided name directly
		CLIENT_NAME="${AUTO_CLIENT_NAME}"
		CLIENT_EXISTS=$(grep -c -E "^### Client ${CLIENT_NAME}\$" "/etc/wireguard/${SERVER_WG_NIC}.conf")
		if [[ ${CLIENT_EXISTS} != 0 ]]; then
			echo -e "${ORANGE}Client '${CLIENT_NAME}' already exists, skipping creation.${NC}"
			return
		fi
		echo "Auto client name: ${CLIENT_NAME}"
	else
		# Interactive: ask for client name
	...
	echo "Auto assigned client IPv4: ${CLIENT_WG_IPV4}"

	# Auto-assign IPv6
	BASE_IP=$(echo "$SERVER_WG_IPV6" | awk -F '::' '{ print $1 }')
	CLIENT_WG_IPV6="${BASE_IP}::${DOT_IP}"
	echo "Auto assigned client IPv6: ${CLIENT_WG_IPV6}"
